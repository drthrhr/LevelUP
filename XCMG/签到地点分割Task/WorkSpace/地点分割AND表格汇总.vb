...
Sub 地点分割_查漏补缺_针对区()
    ...
    Do While fileName <> ""
        If fileName <> ThisWorkbook.Name Then  ' 跳过自身
            ...
            ' 查漏补缺时不再插入
            If found Then
                Dim locationCol As Integer, provinceCol As Integer, cityCol As Integer, adCol As Integer, townCol As Integer, longitudeCol As Integer, latitudeCol As Integer
                ...
                adCol = addressCol + 3    ' “区”列序号
                townCol = addressCol + 4    ' “街道”列序号
                ...
                Dim lastAdValue As String, lastaddressCellValue As String
                ...
                For i = 4 To lastRow
                    ...
                    ' 分割“区”列。先判断本次的“详细地址”是否与上一次的“详细地址”相同。若相同则可以直接使用上一次的街道值，尽量减少插入公式或调用python脚本的次数以加快速度；若不同，则依据情况选择插入公式或调用python脚本。
                    ' 查漏补缺。只有“区”列为空时才进行
                    If ws.Cells(i, adCol).Value = "无效地址" Then
                        If StrComp(addressCellValue, lastaddressCellValue, vbTextCompare) = 0 Then ' 直接使用上一次的街道值
                            ...
                            ws.Cells(i, adCol).Value = lastAdValue
                        ...
                        lastAdValue = ws.Cells(i, townCol).Value
                        wb.Save
                    End If
                Next i
